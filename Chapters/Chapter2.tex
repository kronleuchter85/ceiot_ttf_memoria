...
\begin{itemize}
	\item Lakehouse \cite{link}: que combina las capacidades de un data lake y un data warehouse, permitiendo almacenar y procesar grandes volúmenes de datos estructurados y no estructurados, compatible con Spark, notebooks y consultas SQL.
	\item Warehouse, un entorno de base de datos relacional orientado a análisis empresariales y modelado dimensional, que permite ejecutar consultas SQL sobre los datos almacenados en OneLake.
	\item Data Factory es la herramienta de orquestación de datos que permite construir flujos ETL/ELT mediante pipelines y actividades de copiado,.
	\item Dataflows Gen2 permite realizar transformaciones complejas sobre los datos mediante una interfaz visual sin necesidad de código, utilizando el motor Spark de forma subyacente.
	\item Power BI está integrado de forma nativa en Fabric y permite crear visualizaciones, dashboards e informes interactivos directamente sobre datos almacenados en Lakehouse o Warehouse, sin necesidad de mover los datos.
	\item OneLake, que actúa como repositorio de datos único para todos los servicios de Fabric, asegurando gobierno y seguridad de datos. 
\end{itemize}
